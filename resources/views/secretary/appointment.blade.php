...
<script>
    $(document).ready(function (){
        ...
        var complete = $('#complete').DataTable({
            processing: true,
            serverSide: true,
            ajax: "/secretary/complete-appointment",
            dom: 'frtp',
            pageLength: 10,
            responsive: true,
            columns: [
        ...
        var cancel = $('#cancel').DataTable({
            processing: true,
            serverSide: true,
            ajax: "/secretary/cancelled-appointment",
            dom: 'frtp',
            pageLength: 10,
            responsive: true,
            columns: [
        ...
        var transaction = $('#transaction').DataTable({
            processing: true,
            serverSide: true,
            ajax: "/secretary/transaction-appointment",
            dom: 'frtp',
            pageLength: 10,
            responsive: true,
            columns: [
        ...
        $('.viewpatients').on('shown.bs.modal', function() {
            if (!usertable) {
                    usertable =  $('#users').DataTable({
                        "ajax": "/secretary/appointment/show_user",
                        processing: true,
                        serverSide: true,
                        dom: 'frtp',
                        pageLength: 6,
                        ...
                        "columns": [
                            {data: 'id', name: 'id' , orderable: false, searchable: false},
                            {data: 'fullname', name: 'fullname' ,searchable:true ,orderable: false},
	   ...
	   $('#pendings').on('click', '.cancel', function(e) {
        ...
        $('.update_appointment').on('click', function(e){
            ...
            $.ajax({
                type: "PUT",
                url: "/secretary/appointment/change_status/"+appointcode,
                datatype: "json",
                data: {status:status},
                beforeSend: function(){
                    $(".main-spinner").show();
        ...
        $(document).on('click', '.delete_appointment', function(e){
           ...
           $.ajax({
                type: 'DELETE', 
                url: "/secretary/appointment/delete/"+ appointmentid,
                datatype: "json",
                success: function(response){ 
